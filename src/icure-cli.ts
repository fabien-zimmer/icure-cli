...
import { ContactDto, DelegationDto, FilterDto, ImportResultDto, ListOfIdsDto, PatientDto, UserDto } from '@taktik/icc-api'
...
vorpal
	...
	.action(async function(this: CommandInstance, args: Args) {
		...
		await Promise.all(args.hcpIds.map(async (hcpId: string) => {
			const batchIds = await api.contacticc.matchContactsBy(new FilterDto({
...
vorpal
	...
	.action(async function(this: CommandInstance, args: Args) {
		...
		await api.documenticc.setDocumentAttachment(doc.id, undefined, fs.readFileSync(args.path).buffer)
